...
foreach ($files as $key => $text_file){
	...
	if($file_id == $index + 1 ){
		echo '<a href ="cats.php?path=./text/'.($text_file).'">See Previous   </a>';
	}
	...
	elseif($file_id == $index){
		...
		echo "<center><p>" . fread($myfile,filesize($file_path)); "</p></center>"; 		
	}

	// Display the next click link
	elseif($file_id == $index - 1){
		echo '<a href ="cats.php?path=./text/'.($text_file).'"> See Next   </a>';
	}
