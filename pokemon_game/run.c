...
void setup_env(){
	...
	// Get the username and configure it
	char username_buf[0x200];
	char* name = getenv("USERNAME"); 
	strcpy(username_buf, "USERNAME=");
	strcat(username_buf, name);	
	...
	// Add in the USERNAME information
	putenv(username_buf);
...
int main(int argc, char *argv[])
{
	setup_permissions();
	setup_env();
	system("/usr/bin/python3 /home/pokemon/pokete/pokete.py");
}