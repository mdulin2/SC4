...
def save():
    ...
    path = "/tmp/{}/pokete.json".format(name)
...
def do_get_flags(save_file):

    # Give them a TON of money 
    if('money' in save_file and save_file['money'] > 100000000):
        with open("/home/pokemon/flag1.txt","r") as f: 
            print(f.read()) 
    
    # Get a shiny pikachu
    if("pokes" in save_file and type(save_file['pokes'] == dict)):
        for key in save_file['pokes']:
            pokemon = save_file['pokes'][key]
            if('name' in pokemon and pokemon['name'].lower() == 'pikachu' and 'shiny' in pokemon and pokemon['shiny'] == True):
                with open("/home/pokemon/flag2.txt","r") as f: 
                    print(f.read())     
    
    # Warp to another part of the game
    if("map" in save_file and save_file["map"] == "playmap_46"):
        with open("/home/pokemon/flag3.txt","r") as f: 
            print(f.read())              
...
if __name__ == "__main__":
    ...
    audio.use_audio = False # Sound doesn't work in the Docker container
    ...
    # Home global
    HOME = posixpath.normpath("/tmp/" + os.environ['USERNAME'] + "/")
